		       ...
		       int *channel_id)
{
	...
	param[1].u.value.a = (u64)res->start >> 32;
	param[1].u.value.b = (u64)res->start & 0xffffffff;
...
{
	struct tee_client_device *scmi_device = NULL;
				...
				struct optee_scmi_channel *channel)
{
	...
	if (!channel->shmem) {
		dev_err(cdev, "failed to ioremap SCMI shared memory\n");
		...
		struct optee_scmi_channel *channel)
{
	...
	for (i = 0; i < scmi_optee_desc.max_msg; i++) {
		uintptr_t buffer = (uintptr_t)channel->shmem +
