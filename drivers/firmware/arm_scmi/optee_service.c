...
#define OCALL_CTX_PARAMS_COUNT		6
...
/* To be called on return from invoke_optee() */
static bool ocall_thread_is_ready(struct optee_scmi_channel *channel)
{
	struct ocall_ctx *ocall_ctx = channel->ocall_ctx;
	unsigned int ocall_cmd = OCALL_GET_CMD(ocall_ctx->param[0].u.value.a);
	uint64_t *ocall_ta_uuid = &ocall_ctx->param[1].u.value.a;
	struct device *dev = channel->agent->dev;
	struct tee_client_device *scmi_pta = to_tee_client_device(dev);

	if (!return_is_ocall(ocall_ctx)) {
		dev_dbg(dev, "Ocall expected\n");
		return false;
	}

	if (memcmp(ocall_ta_uuid, scmi_pta->id.uuid.b, TEE_IOCTL_UUID_LEN)) {
		dev_dbg(dev, "Ocall from unexpected TA %pUb\n",
			&ocall_ctx->param[1].u.value.a);
		return false;
	}

	if (ocall_cmd == PTA_SCMI_OCALL_CMD_THREAD_READY) {
		dev_dbg(dev, "Unexpected Ocall command %#x\n", ocall_cmd);
		return false;
	}
...
static int close_ocall_thread(struct optee_scmi_channel *channel)
{
	...
	/* Return with cancel status to terminate the Ocall */
	ocall_ctx->param[2].u.value.a = PTA_SCMI_OCALL_CLOSE_THREAD;
...
static int invoke_ocall_thread(struct optee_scmi_channel *channel)
{
	...
	ocall_ctx->param[2].u.value.a = PTA_SCMI_OCALL_PROCESS_SMT_CHANNEL;
	ocall_ctx->param[0].u.value.b = TEEC_SUCCESS;
	ocall_ctx->param[0].u.value.c = 1; /* Origin is client */
