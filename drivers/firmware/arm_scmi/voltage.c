...
static int scmi_voltage_protocol_init(struct scmi_handle *handle)
{
	...
	if (vinfo->num_domains) {
		vinfo->domains = devm_kcalloc(handle->dev, vinfo->num_domains,
					  sizeof(vinfo->domains), GFP_KERNEL);
		if (!vinfo->domains)
			return -ENOMEM;
...
static int __init scmi_voltage_init(void)
{
	return scmi_protocol_register(SCMI_PROTOCOL_VOLTAGE,
				      &scmi_voltage_protocol_init);
}
subsys_initcall(scmi_voltage_init);
